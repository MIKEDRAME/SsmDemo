                ...
                <div>
                    ...
                    <ul class="list-group list-group-flush m-b-10">
                        <li class="list-group-item d-flex p-l-r-0">
                            ...
                            <div>
                                <p class="m-b-0">New fundProduct registration.</p>
                                <ul class="list-inline small">
...
<nav class="navbar">
    <div class="container-fluid">
        ...
        <div class="header-body">
            ...
            <%--       搜索的表单、包含分页跳转查询     --%>
            <form id="search" action="/SsmDemo/userQueryFundProduct" class="search">
                ...
                <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <form action="/SsmDemo/insertUserFundProductUser" method="post">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalCenterTitle">发起基金定投</h5>
                                ...
                                <div class="modal-body">
                                    <%--             用户id（隐藏输入）                   --%>
                                    <input type="hidden" name="userid" value="${nowUser.id}"/>
                                    <input type="hidden" name="fundid" id="fundid" value=""/>

                                    <div class="form-group mb-4">
                                        定投基金名称：
                                        <div class="col-md-4" id="fundName">
                                            广发多元新兴股票<%--                 默认输入                           --%>
                                        </div>
                                    </div>
                                    <div class="form-group mb-4">
...
<%--                                    <th class="text-right" scope="col">操作</th>--%>
                                ...
                                <c:forEach items="${list}" var="fundProductItem">
                                    <tr>
                                        <th scope="row">${fundProductItem.id}</th>

                                        <td>
                                            <c:if test="${fundProductItem.type==1}">
                                                股票型基金
                                            </c:if>
                                            <c:if test="${fundProductItem.type==2}">
                                                债卷型基金
                                            </c:if>
                                            <c:if test="${fundProductItem.type==3}">
                                                货币型基金
                                            </c:if>
                                            <c:if test="${fundProductItem.type==4}">
                                                混合型基金
                                            </c:if>
                                        </td>

                                        <td>${fundProductItem.code}</td>
                                        <td>${fundProductItem.funddesc}</td>
                                        <td>${fundProductItem.dailygrowth}</td>
                                        <td>${fundProductItem.leastmoney}</td>
                                        <td>${fundProductItem.profit}</td>
                                        <td>${fundProductItem.investerm}</td>
                                        <td class="text-right">
                                            <button type="button"
                                                    onclick="selectedFund('${fundProductItem.id}','${fundProductItem.funddesc}')"<%--onclick之后传入参数--%>
                                                    class="btn btn-primary" data-toggle="modal"
                                                    data-target="#exampleModalCenter">选中定投
                                            </button>
                                        </td>
                                    </tr>
                                </c:forEach>
                ...
                <script>
                    ...
                    //提供一个函数用于获取当前选中定投的基金id和基金名(相当于修改参数)
                    function selectedFund(id, name) {
                        // console.log("id=" + id);
                        // console.log("name=" + name);
                        $('#fundid').attr('value', id);
                        $('#fundName').text(name);
                    }
                </script>
            </div>
